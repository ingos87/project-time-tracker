...
class ClockOutTests : FunSpec({
    ...
    test("clock-out is not possible if there is no config file") {
        ...
        val output = tapSystemOut {
            project.time.tracker.main(arrayOf("clock-out"))
        }
    ...
    test("clock-out is saved empty csv") {
        ...
        output shouldBe "loaded 0 clock events from /tmp/project-time-tracker/test_project_times.csv\n" +
                "wrote 1 events to /tmp/project-time-tracker/test_project_times.csv\n" +
