...
class CostAssessmentRoundingTests : FunSpec({
    ...
    test("no rounding necessary") {
        val input = HashMap<LocalDate, List<BookingPositionItem>>()
        val list1 = listOf(
            BookingPositionItem("topic_1", Duration.parse("PT1H"), emptySet()),
            BookingPositionItem("topic_2", Duration.parse("PT1H30M"), emptySet()),
            BookingPositionItem("topic_3", Duration.parse("PT6H"), emptySet()),
        )
    ...
    test("rounding works within one day") {
        ...
        val list1 = listOf(
            BookingPositionItem("topic_1", Duration.parse("PT1H"), emptySet()),
            BookingPositionItem("topic_2", Duration.parse("PT1H33M"), emptySet()),
            BookingPositionItem("topic_3", Duration.parse("PT6H14M"), emptySet()),
        )
        input[LocalDate.parse("2023-01-03")] = list1


        val expectedOutput = HashMap<LocalDate, List<BookingPositionItem>>()
        val listOut1 = listOf(
            BookingPositionItem("topic_1", Duration.parse("PT1H"), emptySet()),
            BookingPositionItem("topic_2", Duration.parse("PT1H30M"), emptySet()),
            BookingPositionItem("topic_3", Duration.parse("PT6H"), emptySet()),
